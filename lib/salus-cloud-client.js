...
class SalusCloudClient {
  constructor({ email, password, onAuthEvent, onWriteError }) {
    this.email = email;
    this.password = password;
    this.onAuthEvent = typeof onAuthEvent === 'function' ? onAuthEvent : null;
    this.onWriteError = typeof onWriteError === 'function' ? onWriteError : null;
  ...
  async authenticate() {
    ...
    this._applySession(result);
    this._emitAuthEvent('full_login');
  }

  _emitAuthEvent(type, detail) {
    ...
    try {
      ...
      // Evidence collection must never break the client.
    }
  }
  ...
  _applySession(session) {
    ...
    if (typeof session.getRefreshToken === 'function') {
      this.refreshToken = session.getRefreshToken() || this.refreshToken;
    }

    // getExpiration() is epoch seconds from the JWT itself; trust the shorter-lived token.
    const expirationSec = Math.min(
      session.getAccessToken().getExpiration(),
      session.getIdToken().getExpiration(),
    );
    this.tokenExpiresAt = expirationSec * 1000 - TOKEN_REFRESH_SKEW_MS;
  }

  async refreshSession() {
    if (!this.cognitoUser || !this.refreshToken) {
      throw new Error('No Cognito session available to refresh');
    }

    const session = await new Promise((resolve, reject) => {
      this.cognitoUser.refreshSession(this.refreshToken, (err, newSession) => {
        if (err) reject(err);
    ...
    this._emitAuthEvent('token_refresh');
  }
  ...
  async ensureAuth() {
    ...
    if (this.cognitoUser && this.refreshToken) {
      try {
        ...
        // Refresh token revoked or expired; fall back to a full login.
        this._emitAuthEvent('refresh_failed', error?.message || String(error));
      }
    }
  ...
  async request(method, endpoint, body, isRetry = false) {
    ...
    if (!response.ok) {
      if ((response.status === 401 || response.status === 403) && !isRetry) {
        // Tokens may have been invalidated server-side before our local expiry.
        this._emitAuthEvent('auth_retry', `HTTP ${response.status} on ${endpoint}`);
  ...
  _emitWriteError(deviceCode, properties, error) {
    ...
    try {
      ...
      // Reporting must never break the write path.
    }
  }
    ...
    try {
      ...
      if (message.includes('Connection closed') || message.includes('disconnect')) {
        try {
          ...
          this._emitWriteError(deviceCode, properties, retryError);
          await this.disconnectMqtt();
          throw retryError;
        }
      }
      this._emitWriteError(deviceCode, properties, error);
      await this.disconnectMqtt();
      throw error;
    }
  }
  ...
  /** Switch a smart plug / relay (SPE600 family) on or off. */
  async setOnOff(deviceCode, on, deviceIndexHint) {
    const properties = { 'ep9:sOnOffS:SetOnOff': on ? 1 : 0 };
    await this.updateDeviceShadow(deviceCode, properties, deviceIndexHint);
  }
